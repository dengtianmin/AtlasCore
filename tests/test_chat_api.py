...
from app.integrations.dify.exceptions import DifyConfigurationError, DifyRequestError, DifyTimeoutError
...
class StubDifyClient:
    ...
    def chat(self, payload):
        ...
        if self.mode == "error":
            raise DifyRequestError("upstream error")
        return DifyChatResponse(
            answer=f"Dify answer: {payload.query}",
            source="dify",
            sources=[{"title": "doc-1", "snippet": "AtlasCore context", "source": "knowledge"}],
            retrieved_context="AtlasCore context",
            session_id=payload.session_id,
            provider_message_id="msg-123",
            metadata={"retriever_resource_count": 1, "total_tokens": 42},
        )
...
def test_chat_message_and_feedback_flow(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert message.sources == ["doc-1"]
        assert message.retrieved_context == "AtlasCore context"
        ...
        assert stored["retrieved_context"] == "AtlasCore context"
        assert stored["provider_message_id"] == "msg-123"
...
def test_chat_message_returns_controlled_error_when_dify_not_configured(monkeypatch, tmp_path):
    ...
    try:
        try:
            send_message(ChatMessageRequest(question="配置缺失怎么办？", session_id="session-a"), db=db)
...
def test_chat_message_returns_controlled_error_when_dify_times_out(monkeypatch, tmp_path):
    ...
    try:
        try:
            send_message(ChatMessageRequest(question="超时测试", session_id="session-b"), db=db)
        except HTTPException as exc:
...
def test_chat_message_returns_controlled_error_when_dify_fails(monkeypatch, tmp_path):
    ...
    try:
        try:
            send_message(ChatMessageRequest(question="失败测试", session_id="session-c"), db=db)
        except HTTPException as exc:
